 ...
 */
function getEpisodes(episodes): EpisodeData[] {
    // Episode Data
    const EP_DATA: EpisodeData[] = [];
 ...
 */
function outputEpToText(EP_LIST: EpisodeData[], fileName: string): Boolean {
    if (!EP_LIST.length) return false;  // Failed to Save if EP_LIST is empty
